...
export async function GET(request: NextRequest) {
  ...
  conditions.push(`("endDate" IS NULL OR "endDate" >= NOW())`);
  ...
  const [countResult, listings] = await Promise.all([
    ...
    prisma.$queryRawUnsafe<Record<string, unknown>[]>(
      `SELECT l."id", l."title", l."slug", l."type", l."description", l."image",
       ...
       WHERE ${whereClause.replace(/"searchVector"/g, 'l."searchVector"').replace(/"status"/g, 'l."status"').replace(/"expiresAt"/g, 'l."expiresAt"').replace(/"endDate"/g, 'l."endDate"').replace(/"type"/g, 'l."type"').replace(/"level"/g, 'l."level"').replace(/"price"/g, 'l."price"').replace(/"providerId"/g, 'l."providerId"')}
